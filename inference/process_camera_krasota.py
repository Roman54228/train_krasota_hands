    ...
    def process_frame(self, image, depth=None):
        ...
        for i, box in enumerate(results[0].boxes.xyxy[:2]):
            ...
            for j, pt in enumerate(pts):
                ...
                self.sender.send(address=f"/bboxes/bbox_{i}", data=[j, pt[0], pt[1], pt[2]])
        ...
        for i, box in enumerate(results[0].boxes.xyxy[:2]):
            ...
            for p_id, (x_norm, y_norm) in enumerate(preds):
                ...
                # Отправка каждой keypoint отдельно
                self.sender.send(address=f"/hand_{i}", data=[p_id, real_x, real_y, real_z])
